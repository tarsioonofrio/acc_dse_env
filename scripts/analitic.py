import sys
sys.path.insert(1, "../apps/imports")
...
sim_rtl_log_path = "../sim_rtl/results/layer" + str(LAYER) + "/" + str(ARRAY_TYPE) + "_" + str(DATAFLOW_TYPE) + "_lat" + str(MEM_LAT) + "_log.txt"
psynth_results_path = "../psynth/28nm/results/" + str(CLOCK_PERIOD) + "ns/" + str(INPUT_SIZE) + "bit/" + str(ARRAY_TYPE) + "_" + str(DATAFLOW_TYPE) + "/layer" + str(LAYER) + "/reports/" + str(ARRAY_TYPE) + "_" + str(DATAFLOW_TYPE) + "_area.txt"
power_analyses_results_path = "../power_analyses/28nm/results/" + str(CLOCK_PERIOD) + "ns/" + str(INPUT_SIZE) + "bit/" + str(ARRAY_TYPE) + "_" + str(DATAFLOW_TYPE) + "/lat" + str(MEM_LAT) + "/layer" + str(LAYER) + "/reports/activity_avg_power.txt"
power_analyses_hier_results_path = "../power_analyses/28nm/results/" + str(CLOCK_PERIOD) + "ns/" + str(INPUT_SIZE) + "bit/" + str(ARRAY_TYPE) + "_" + str(DATAFLOW_TYPE) + "/lat" + str(MEM_LAT) + "/layer" + str(LAYER) + "/reports/activity_hier_power.txt"
...
if (DATAFLOW_TYPE == "ws" or DATAFLOW_TYPE == "ws_buf"):
  ...
  inmem_read = ((layer_dimension+5)*6*CNN_INPUT_N*CNN_FILTER_N) + CNN_FILTER_N + (((CNN_FILTER_D*CNN_FILTER_D)+1)*CNN_INPUT_N*CNN_FILTER_N) + (6*layer_dimension*layer_dimension*CNN_INPUT_N*CNN_FILTER_N) 
  ...
  # Compute input memory read
  inmem_read = (layer_dimension*layer_dimension)*9*CNN_INPUT_N + CNN_FILTER_N + (CNN_FILTER_N*CNN_FILTER_D*CNN_FILTER_D*CNN_INPUT_N)
  ...
  if MEM_TYPE == "sram":
    ...
    term2 = ((layer_dimension*layer_dimension)*9*CNN_INPUT_N + CNN_FILTER_N + (CNN_FILTER_N*CNN_FILTER_D*CNN_FILTER_D*CNN_INPUT_N))*(1+MEM_LAT)
  ...
  # Compute input memory read
  inmem_read = (18*CNN_FILTER_N) + ((layer_dimension*layer_dimension)*18*CNN_INPUT_N*CNN_FILTER_N) + (CNN_FILTER_N)
...
# Compute read an write of ofmap memory
if (DATAFLOW_TYPE == "ws_buf" or DATAFLOW_TYPE == "is_slice_buf" or DATAFLOW_TYPE == "is_slice_buf_hier" or DATAFLOW_TYPE == "os"):
  ofmap_read = 0
...
TOTAL_POWER = (float(MEM_READ_ENERGY*(inmem_read+ofmap_read))) + (float(MEM_WRITE_ENERGY*(ofmap_write))) + ((float((POWER-POWER_BUF) + output_buf_power)*int(cycles))/1000000)
