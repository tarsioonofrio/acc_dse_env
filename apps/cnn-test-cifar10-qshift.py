...
def main():
    ...
    output = generate_rtl.forward(10, generate_rtl.total_layers-1, generate_rtl.map_gold_torch)

    model2 = pytorch_models_lower[cnn_config["name"]](cnn_config)
    model2.load_state_dict(torch.load(cnn_output_path / 'model.pth'))
    output2 = model2(dataloader[0][0].unsqueeze(0))
    print()
