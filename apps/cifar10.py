...
if (not (os.path.exists("./model"))):
    model = tflib.build_neural_network(featureShape, filter_channel, filter_dimension, stride_h, stride_w, input_h,
...
envlib.generate_generic_file(X_SIZE, C_SIZE, FILTER_WIDTH, CONVS_PER_LINE, MEM_SIZE, INPUT_SIZE, CARRY_SIZE, CLK_PERIOD,
...
envlib.generate_tcl_generic(X_SIZE, C_SIZE, FILTER_WIDTH, CONVS_PER_LINE, MEM_SIZE, INPUT_SIZE, CARRY_SIZE, CLK_PERIOD,
                            DATAFLOW_TYPE, ARRAY_TYPE, stride_h[LAYER], filter_channel[LAYER], LAT, shift_bits, LAYER)

# Generate VHDL tensorflow package
envlib.generate_tf_vhd_pkg(modelDict, shift, input_size, filter_dimension, filter_channel, layer_dimension,
                           input_channel, x_test, y_test, stride_h, stride_w, 1, LAYER)

# Generate VHDL gold output package
envlib.generate_gold_vhd_pkg(modelDict, shift, input_size, filter_dimension, filter_channel, layer_dimension,
                             input_channel, x_test, y_test, stride_h, stride_w, 1, LAYER)
