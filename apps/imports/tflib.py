...
def build_neural_network(featureShape, filter_channel, filter_dimension, stride_h, stride_w, input_h, input_w, input_c,
    ...
    # Add layers
    model.add(keras.layers.Conv2D(filter_channel[0], (filter_dimension[0], filter_dimension[0]),
                                  strides=(stride_h[0], stride_w[0]), activation='relu',
                                  input_shape=(input_h, input_w, input_c)))

    for i in range(1, n_conv_layers):
        model.add(keras.layers.Conv2D(filter_channel[i], (filter_dimension[i], filter_dimension[i]),
                                      strides=(stride_h[i], stride_w[i]), activation='relu'))
