...
def generate_generic_file(X_SIZE, C_SIZE, FILTER_WIDTH, CONVS_PER_LINE, MEM_SIZE, INPUT_SIZE, CARRY_SIZE, CLK_PERIOD, STRIDE, N_FILTER, DATAFLOW_TYPE, LAT, SHIFT, IN_DELAY):
   CLK_HALF = CLK_PERIOD/2
   RST_TIME = CLK_HALF*5
   
   # Open file
   f = open("generic_file.txt", "w+")
   f.write("-gN_FILTER=" + str(N_FILTER) + " -gSTRIDE=" + str(STRIDE) + " -gX_SIZE=" + str(X_SIZE) + " -gFILTER_WIDTH=" + str(FILTER_WIDTH) + " -gCONVS_PER_LINE=" + str(CONVS_PER_LINE) + " -gMEM_SIZE=" + str(MEM_SIZE) + " -gINPUT_SIZE=" + str(INPUT_SIZE) + " -gCARRY_SIZE=" + str(CARRY_SIZE) + " -gCLK_PERIOD=" + str(CLK_HALF) + "ns" + " -gRST_TIME=" + str(RST_TIME) + "ns" + " -gRISE_START=" + str(CLK_HALF*2.0 + RST_TIME + IN_DELAY) + "ns" + " -gFALL_START=" + str(CLK_HALF*4.0 + RST_TIME + IN_DELAY) + "ns" + " -gIN_DELAY=" + str(IN_DELAY) + "ns" + " -gLAT=" + str(LAT) + " -gN_CHANNEL=" + str(C_SIZE) + " -gSHIFT=" + str(int(SHIFT)) + "\n")
   f.close()
   
def generate_tcl_generic(X_SIZE, C_SIZE, FILTER_WIDTH, CONVS_PER_LINE, MEM_SIZE, INPUT_SIZE, CARRY_SIZE, CLK_PERIOD, DATAFLOW_TYPE, ARRAY_TYPE, STRIDE, N_FILTER, LAT, SHIFT, LAYER):
   # Open file
   ...
   f.write("set parameters {{N_FILTER " + str(N_FILTER) + "}" + " {N_CHANNEL " + str(C_SIZE) + "}" + " {STRIDE " + str(STRIDE) + "}" + " {X_SIZE " + str(X_SIZE) + "}" + " {FILTER_WIDTH " + str(FILTER_WIDTH) + "}" + " {CONVS_PER_LINE " + str(CONVS_PER_LINE) + "}" + " {MEM_SIZE " + str(MEM_SIZE) + "}" + " {INPUT_SIZE " + str(INPUT_SIZE) + "}" + " {CARRY_SIZE " + str(CARRY_SIZE) + "}" + " {SHIFT " + str(int(SHIFT)) + "}}" + "\n")
   f.close()

def generate_tf_vhd_pkg(modelDict,shift,input_size,filter_dimension,filter_channel,layer_dimension,input_channel,testSet,testLabel,stride_h,stride_w,testSetSize,layer):
   # Auxiliar variables
   cont = 0
     ...
     if modelDict[layerId]["type"] == "Conv2D":
       if layerId == layer:
         ...
         for filterId in modelDict[layerId]["filter"]:
           string_bias = string_bias + str(int(modelDict[layerId]["filter"][filterId]["bias"]*shift*shift)) + ","
         f.write("\n")
         f.write(string_bias)
         string_bias = "{"
         f.write("\n\n")
...
def generate_gold_vhd_pkg(modelDict,shift,input_size,filter_dimension,filter_channel,layer_dimension,input_channel,testSet,testLabel,stride_h,stride_w,testSetSize,layer):
   # Dataset test variables
   cont_match = 0
   aux_idx_range = 0
